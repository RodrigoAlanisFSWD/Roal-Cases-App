...
export const DashboardOrderDetail: FC<OrderDetailProps> = ({ order }) => {
  ...
  return (
    <div>
        ...
        <div className='flex border-t border-gray-300 p-5'>
                <Select items={orderStates.filter((state: SelectItemType) => state.key !== status)} placeholder="Estado" selectedItem={selectedState} onSelect={(item) => setSelectedState(item)} width="w-[450px]" className='w-[450px]' />
                <Button text='Actualizar' className="ml-5 h-[55px] w-[250px]" onClick={async () => {
                    if (selectedState) {
