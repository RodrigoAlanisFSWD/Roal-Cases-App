...
export const Counter: FC<CounterProps> = ({ count, setCount }) => {
    return (
        <div className='flex items-center justify-center w-[150px] bg-background h-[45px] rounded-sm'>
            <div className='w-1/3 h-full flex justify-center items-center text-dark text-xl hover:bg-gray-200 transition-all duration-300 cursor-pointer'>
            ...
            <div className='w-1/3 h-full flex justify-center items-center text-dark text-xl hover:bg-gray-200 transition-all duration-300 cursor-pointer'>
                <FontAwesomeIcon icon={faPlus} onClick={() => setCount(count + 1)} className="select-none" />
            </div>
        </div>
    )
}
